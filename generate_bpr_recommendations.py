...
def main():
    ...
    print(f"Generating recommendations for {len(valid_uids)} users...")
    ...
    # Save to TSV
    rows = []
    for user_tok, items, scores_list in zip(user_tokens, item_tokens, topk_scores):
        rows.append({
            "user_id": user_tok,
    ...
    print(f"✅ Saved BPR top-100 per user to {out_path}")
