...
def main():
    ...
    # Load ItemKNN recommendations
    print("Loading ItemKNN recs...")
    recs = pd.read_csv(ITEMKNN_RECS, sep="\t")
    ...
    # Process each lambda
    for lam in LAMBDAS:
        print(f"Calibration for Lambda {lam}...")
        results = []
        ...
        save_path = f"outputs/03_calibration/user_top10_itemknn_calibrated_{SCORE_TYPE}_lambda_{lam}.tsv"
        out_df.to_csv(save_path, sep="\t", index=False)
        print(f"Saved: {save_path}")

    print("ItemKNN calibration done.")
